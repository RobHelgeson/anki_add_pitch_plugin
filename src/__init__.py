...
from .parse import get_field_and_reading, just_hiragana
...
def set_pitch_automatically(editor):
    ...
    if not editor.web.editor.currentField:
        showInfo("No field selected.", title="No field selected.")
        return
    ...
    for fld, val in editor.note.items():
        ja_expr, rd_expr = get_field_and_reading(editor.mw.col.media.escape_images(val))
        if ja_expr is None:
            # no Japanese, next
            continue
        if expr_guess is None:
            ...
            expr_guess = ja_expr
        if rd_expr is not None:
            reading_guess = rd_expr
            break
        all_hira = just_hiragana(ja_expr)
        if all_hira and reading_guess is None:
            # if first continuous block is all hiragana, treat as reading
            # and don’t override afterwards
            reading_guess = all_hira
...
def set_pitch(editor, hira, LH_patt):
    ...
    # get note data
    data = [
        (fld, editor.mw.col.media.escape_images(val))
        for fld, val in editor.note.items()
    ]
