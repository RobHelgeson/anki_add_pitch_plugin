...
def get_acc_patt(expr_guess, reading_field, dicts):
    ...
    all_hira = just_hiragana(reading_field)
    if all_hira:
...
def add_pitch(acc_dict, note_ids, expr_idx, reading_idx, output_idx):
    ...
    for nid in note_ids:
        ...
        expr_field, reading_field = get_field_and_reading(note[expr_fld].strip())
        if not reading_field:
            reading_field = reading_fld
